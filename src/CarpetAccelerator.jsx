...
const CarpetAccelerator = () => {
  ...
  const [formData, setFormData] = useState({
    ownerName: '', mobileNo: '', district: '', organisationName: '', orgType: '',
    exportSituation: '', productionCapacity: '', employees: '', workingProcess: '',
    valueAddedProducts: '', socialMediaOrGI: ''
  });
  ...
  const [files, setFiles] = useState({
    aadharCard: null, panCard: null, productsImage: null, brochure: null, socialMediaImage: null, otherDocs: null
  });

  // Handlers
  const handleInputChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const handleFileChange = (e, field) => setFiles({ ...files, [field]: e.target.files[0] });
            ...
            <form className="p-8 md:p-12" onSubmit={submitApplication}>
                ...
                <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="text" name="ownerName" value={formData.ownerName} onChange={handleInputChange} required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="Enter owner's name" />
                    </div>
                    <div>
                      ...
                      <input type="tel" name="mobileNo" value={formData.mobileNo} onChange={handleInputChange} required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="+91" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-semibold text-gray-700 mb-2">District (जिला) <span className="text-red-500">*</span></label>
                      <input type="text" name="district" value={formData.district} onChange={handleInputChange} required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="Enter district" />
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input type="text" name="organisationName" value={formData.organisationName} onChange={handleInputChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="Enter organisation name" />
                    </div>
                    ...
                    <div>
                      ...
                      <select name="orgType" value={formData.orgType} onChange={handleInputChange} required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none bg-white">
                    ...
                    <div>
                      ...
                      <select name="exportSituation" value={formData.exportSituation} onChange={handleInputChange} required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none bg-white">
                    ...
                    <div>
                      ...
                      <input type="text" name="productionCapacity" value={formData.productionCapacity} onChange={handleInputChange} required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="e.g. 1000 units/month" />
                    </div>

                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Number of Employees (कर्मचारियों की संख्या)</label>
                      <input type="number" name="employees" value={formData.employees} onChange={handleInputChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="e.g. 50" />
                    </div>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea name="workingProcess" value={formData.workingProcess} onChange={handleInputChange} required rows="2" className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none resize-none" placeholder="Describe your working process..."></textarea>
                    </div>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input type="text" name="valueAddedProducts" value={formData.valueAddedProducts} onChange={handleInputChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="Mention value-added products" />
                    </div>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input type="text" name="socialMediaOrGI" value={formData.socialMediaOrGI} onChange={handleInputChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#a67c00] outline-none" placeholder="Links or GI tag info" />
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                          "Registration / Gumasta etc.", "GST Registration", "Bank Account", 
                          "Import Export Certificate (IEC)", "PAN Card", "Other"
                        ].map(doc => (
                          <label key={doc} className="flex items-start gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" value={doc} onChange={(e) => handleCheckboxChange(e, docsAvailable, setDocsAvailable)} className="w-4 h-4 mt-0.5 text-[#581c24] focus:ring-[#a67c00] rounded" />
                            <span>{doc}</span>
                          </label>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                          "Product (Designing / Enhancement)", "Process (प्रक्रिया)", "Packing (पैकिंग)", 
                          "Documentation (दास्तावेज़)", "Marketing and Branding", "Selling and Supply Chain",
                          "Market Research (Product / Intelligence)", "Export Related (निर्यात संबंधी)", "Other"
                        ].map(need => (
                          <label key={need} className="flex items-start gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" value={need} onChange={(e) => handleCheckboxChange(e, trainingNeeds, setTrainingNeeds)} className="w-4 h-4 mt-0.5 text-[#581c24] focus:ring-[#a67c00] rounded" />
                            <span>{need}</span>
                          </label>
                        ))}
                      </div>
                  ...
                  <div className="pt-6 border-t border-gray-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <input type="file" required onChange={(e) => handleFileChange(e, 'aadharCard')} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#f5ebd9] file:text-[#581c24] hover:file:bg-[#e6d5bc] transition-all border border-gray-300 rounded-lg" />
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Pan Card (पैन कार्ड चित्र)</label>
                        <input type="file" onChange={(e) => handleFileChange(e, 'panCard')} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#f5ebd9] file:text-[#581c24] hover:file:bg-[#e6d5bc] transition-all border border-gray-300 rounded-lg" />
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Products manufactured (निर्मित उत्पाद)</label>
                        <input type="file" onChange={(e) => handleFileChange(e, 'productsImage')} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#f5ebd9] file:text-[#581c24] hover:file:bg-[#e6d5bc] transition-all border border-gray-300 rounded-lg" />
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Company's brochure or catalogue</label>
                        <input type="file" onChange={(e) => handleFileChange(e, 'brochure')} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#f5ebd9] file:text-[#581c24] hover:file:bg-[#e6d5bc] transition-all border border-gray-300 rounded-lg" />
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Image of Social Media Page or GI Tag</label>
                        <input type="file" onChange={(e) => handleFileChange(e, 'socialMediaImage')} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#f5ebd9] file:text-[#581c24] hover:file:bg-[#e6d5bc] transition-all border border-gray-300 rounded-lg" />
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Other Above Mentioned documents (PDF)</label>
                        <input type="file" accept=".pdf" onChange={(e) => handleFileChange(e, 'otherDocs')} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#f5ebd9] file:text-[#581c24] hover:file:bg-[#e6d5bc] transition-all border border-gray-300 rounded-lg" />
                      </div>
              ...
              {activeStep < 4 && (
                <div className="mt-10 flex justify-between pt-6 border-t border-gray-100">
                  ...
                  {activeStep < 3 ? (
                    <button type="button" onClick={handleNextStep} className="flex items-center gap-2 px-8 py-3 font-semibold rounded-lg text-white bg-[#581c24] hover:bg-[#43151b] shadow-md transition-all">
                      Next <ArrowRight className="w-4 h-4" />
                    </button>
                  ) : (
                    <button type="submit" disabled={isSubmitting} className="flex items-center gap-2 px-8 py-3 font-semibold rounded-lg text-white bg-[#a67c00] hover:bg-[#8e6a00] shadow-md transition-all disabled:opacity-70">
...
{activeStep === 4 && (
  <div id="success-section" className="text-center py-12 animate-fade-in">
    ...
    <p className="text-lg text-gray-600 mb-8 max-w-lg mx-auto">
      Your application has been submitted successfully to the Carpet Accelerator program.
      Please join our official WhatsApp group for important updates and next steps.
    </p>
